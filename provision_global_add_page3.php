			...
			echo '<div class="row">
				<div class="col-md-12">
					<div class="panel panel-default">
						<div class="panel-heading"><b>'.device_type_to_name($chosen_model).', '.$chosen_group_soft.'</b><br><br><a href="provision_global_add_page2.php?type='.$chosen_model.'&group='.$chosen_group.'"><-- Back to Sections</a></div>
					</div>
				</div>
			</div>';
				...
				while($a3=$q3->fetch())
				{
					...
					//Right Side:
					echo '<form action="provision_global_submit_panel.php?type='.$chosen_model.'&group='.$chosen_group.'" method="POST" target="feedback_panel_1">';
					if ($a3['setting_variable_type']==1)
					{
						//GET DEFAULT
						...
						//GET OUR ENTRY
						$qE=$con->query("SELECT * FROM provisioner_global_settings WHERE device_type=".$chosen_model." AND device_group=".$chosen_group." AND setting_variable_name='".$a3['setting_variable_name']."'");
						...
						echo 	'<div class="col-md-6">
									...
									for($i=1;$i<=20;$i+=1)
									{
										$this_option=urldecode($a3['setting_option_'.$i]);
					...
					elseif ($a3['setting_variable_type']==2)
					{
						//GET DEFAULT
						//$qD=$con->query("SELECT * FROM provisioner_device_defaults WHERE device_type=".$chosen_model." AND setting_variable_name='".$a3['setting_variable_name']."'");
						//if ($qD && $qD->rowCount()) {$aD=$qD->fetch(); $defaultValue=$aD['setting_option_1'];} else {$defaultValue="";}
						...
						if ($myValue==="") {$myValue=$a3['setting_option_1'];}
						//FREETYPE
						//Attach group inheritance
						echo 	'<div class="col-md-6">
									<input name="'.$a3['setting_variable_name'].'" value="'.str_replace("!PLUS!", "+", urldecode($myValue)).'" onkeyup="makeButtonBlink(1);" placeholder="'.inherit_group($chosen_model, $chosen_group, $a3['setting_variable_name']).'">
								</div>';
					}
					else
					{
