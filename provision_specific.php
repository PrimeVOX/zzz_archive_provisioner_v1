...
<div class="content">
	...
	<div class="page-header">
		<h1 class="title">Individual Settings</h1>
		<ol class="breadcrumb">
		...
		<div class="row">
			<div class="col-md-12">
				<div class="panel panel-default">
					<div class="panel-heading">Active Devices</div>
							...
							if ($con)
							{
								...
								if ($q1 && $q1->rowCount()>0)
								{
							        ...
							        while($a1=$q1->fetch())
							        {
								        ...
								        if ($a1['device_group']==0)
								        {
									        $group_name="No Group";
										}
										else
										{
									        $q2 = $con->query("SELECT * FROM provisioner_group_definitions WHERE group_number=".$a1['device_group']);
									        if ($q2 && $q2->rowCount()>0)
									        {
												$a2=$q2->fetch();
												$group_name=urldecode($a2['group_name']).' (Group '.$a1['device_group'].')';
										    }
										    else
										    {
										...
										echo '<tr><td><a href="provision_specific_add.php?mac_address='.strtoupper($a1['mac_address']).'&group='.$a1['device_group'].'">'.urldecode($a1['soft_name']).'</a></td><td><a href="provision_specific_add.php?mac_address='.strtoupper($a1['mac_address']).'&group='.$a1['device_group'].'">'.mac_format($a1['mac_address']).'</a></td><td>'.$group_name.'</td><td>'.device_type_to_name($a1['device_type']).'</td><td>'.$np.'</td></tr>';
